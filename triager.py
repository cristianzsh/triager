...
def _enable_windows_vt_mode() -> bool:
    ...
    try:
        ...
        # essentially every modern Windows install). That truncates the
        # real handle value, so every GetConsoleMode/SetConsoleMode call
        # afterward silently operates on a corrupted handle and fails.
...
def _appdata_tools_dir() -> Path:
    appdata = os.environ.get("APPDATA") or str(Path.home() / "AppData" / "Roaming")
    return Path(appdata) / "triager" / "tools"


def _ensure_tools_dir() -> Path:
    """
    build.bat appends a zip of tools/ directly onto the end of
    the compiled .exe. The PyInstaller bootloader's own per-launch
    extraction only ever has to unpack the actual Python runtime,
    a fixed cost that doesn't grow as more forensic utilities get
    added to tools/ over time.
    """
    if not getattr(sys, "frozen", False):
        return Path(__file__).resolve().parent / "tools"

    target = _appdata_tools_dir()
    ...
    try:
        with zipfile.ZipFile(sys.executable) as zf:
            names = zf.namelist()
            if not names:
                return target  # nothing appended; caller just won't find any tools
            log_info(f"First run: extracting bundled tools to {target}...")
            target.mkdir(parents=True, exist_ok=True)
            zf.extractall(target)
    except (zipfile.BadZipFile, FileNotFoundError, OSError):
        return target  # nothing appended (or unreadable); no tools available

    marker.write_text("ok", encoding="utf-8")
    log_success("Tools extracted. Future runs will start instantly.")
    return target
...
def _parser_token_matches(user_token: str, tool_name: str) -> bool:
    ...
    if user_norm == tool_norm:
        return True
    # Prefix match, and only for tokens with enough characters to be a
    # deliberate identifier, not an accidental single letter.
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        default=[],
        metavar="NAME",
        help="Skip a parser (comma-separated, or repeat the flag). See --list-parsers.",
    )
    ap.add_argument(
        "--list-parsers",
        action="store_true",
    ...
    ap.add_argument("--port", type=int, default=8000, help="Port to listen on, used with --web.")
    ap.add_argument(
        "--host", default="127.0.0.1",
        help="Address to bind to, used with --web (default: 127.0.0.1). "
             "Anything else automatically serves over HTTPS with a self-signed cert.",
    )
    ap.add_argument("--no-browser", action="store_true", help="With --web, don't automatically open a browser tab.")
    ap.add_argument(
        "--data-dir", default=None,
        help="With --web, override where case data/uploads/the app database are stored.",
    )
...
def _ensure_web_backend_on_path() -> None:
    """When frozen, the web backend package is bundled directly into this
    same exe (see build.bat's --paths), so normal import machinery already
    finds it -- no sys.path change needed. When run from source, it lives
    at web/backend relative to this script."""
    if getattr(sys, "frozen", False):
        return
    backend_dir = Path(__file__).resolve().parent / "web" / "backend"
...
def _ensure_self_signed_cert(cert_dir: Path, host: str) -> tuple[Path, Path]:
    ...
    if cert_path.exists() and key_path.exists():
        try:
            ...
            if existing.not_valid_after_utc > _dt.datetime.now(_dt.timezone.utc) + _dt.timedelta(days=1):
...
def _wait_for_web_server(url: str, timeout: float = 20.0, verify_tls: bool = True) -> bool:
    import ssl
...
def _print_web_banner(host: str, port: int, https: bool) -> None:
    display_host = "localhost" if host in ("127.0.0.1", "0.0.0.0") else host
    scheme = "https" if https else "http"
    url = f"{scheme}://{display_host}:{port}"

    print()
    print(C.INFO + ("=" * 60) + C.RESET if USE_COLOR else "=" * 60)
    print("  Triager Console")
    print(("=" * 60))
    ...
    if _is_loopback(host):
        ...
        print(f"  NOTE: bound to {host}, not just localhost!")
        print("  Using a self-signed TLS certificate -- your browser will warn")
        print("  about it once; that's expected for a self-signed cert.")
    print()


def _run_web_mode(args: argparse.Namespace) -> int:
    """Starts the same FastAPI/uvicorn server as the standalone web app
    used to, now reachable from this same binary via --web instead of a
    separate TriagerWeb.exe."""
    ...
    try:
        ...
        log_error(f"Web mode isn't available in this build: {ex}")
        return 1
    ...
    _print_web_banner(args.host, args.port, https)
    ...
    if not args.no_browser:
        ...
        def _open_when_ready():
            if _wait_for_web_server(health_url, verify_tls=not https):
                webbrowser.open(browser_url)

        _threading.Thread(target=_open_when_ready, daemon=True).start()

    try:
        uvicorn.run(app, host=args.host, port=args.port, log_level="info", use_colors=USE_COLOR, **ssl_kwargs)
